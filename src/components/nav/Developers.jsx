          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-4">
            ...
            >
              <div className="relative h-36">
                <Image src="/dashboard/land.jpg" alt="All Developers" fill className="object-cover" />
            ...
            {cardsToRender.map(({ cfg, dev }, i) => {
              const href = dev ? `/off-plan?developer=${encodeURIComponent(dev.id)}` : '#';
              const bg = cfg?.img || fallbackImg;
              const label = cfg?.label || 'Developer';
              ...
              return (
                ...
                >
                  <div className="relative h-36">
                    ...
                    {/* Developer logo pinned top-left (if available) */}
                    {dev?.logoUrl && (
                      <div className="absolute top-2 left-2 rounded-md bg-white/90 backdrop-blur px-2 py-1 shadow">
                        <Image
                          src={dev.logoUrl}
                          alt={`${label} logo`}
                          width={76}
                          height={24}
                          className="object-contain w-[76px] h-[24px]"
                        />
                      </div>
                    )}
                  </div>
