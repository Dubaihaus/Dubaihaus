      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-8 px-4 pb-10 pt-16 lg:flex-row lg:items-center">
        <div className="flex-1">
          ...
          <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-900 sm:text-4xl">
            Discover UAE&apos;s leading developers
          </h1>
          <p className="mt-3 max-w-xl text-sm text-slate-600">
            Explore trusted names like Emaar, Damac, Sobha, Aldar and more.
            Filter by name and quickly jump into off-plan projects built by each
            developer.
          </p>
          {typeof total === "number" && total > 0 && (
            <p className="mt-4 text-xs font-medium text-slate-500">
              Currently listing{" "}
              <span className="text-[var(--color-brand-sky)]">{total}</span>{" "}
              developers.
            </p>
          )}
        </div>
          ...
          <div className="rounded-3xl border border-white/80 bg-white/80 p-4 shadow-[0_18px_60px_rgba(15,23,42,0.12)] backdrop-blur-xl">
            ...
            <div className="mt-3 space-y-3">
              ...
              <p className="text-[11px] text-slate-500">
                Tip: start typing to instantly filter the grid below.
              </p>
            </div>
          </div>
        </div>
